...
autocmd BufWritePre * :%s/\s\+$//e "<Bar> :set ff=unix
...
repos=`cat .git/config | grep url | head -n 1 | cut -d '=' -f2`
branch=`git branch | grep \* | tr -d '* '`
commitid="`git log | head -n 1 | cut -d ' ' -f2`"

if [ "`echo $repos | grep gerrithost2`" ] ; then
	echo "git push $repos HEAD:refs/for/$branch"
	#git push --receive-pack='git receive-pack --reviewer donghee.yang@samsung.com' $repos HEAD:refs/for/$branch
	git push $repos HEAD:refs/for/$branch
	firefox http://172.21.111.186/gerrit/#q,$commitid,n,z
elif [ "`echo $repos | grep rsa`" ] ; then
	echo "git push $repos HEAD:refs/for/$branch"
	git push --receive-pack='git receive-pack --reviewer donghee.yang@samsung.com' $repos HEAD:refs/for/$branch
	#git push $repos HEAD:refs/for/$branch
	firefox http://172.21.17.55/review/#q,$commitid,n,z
elif [ "`echo $repos | grep review.tizen.org`" ] ; then
	echo "git push $repos HEAD:refs/for/$branch"
	#git push --receive-pack='git receive-pack --reviewer donghee.yang@samsung.com' $repos HEAD:refs/for/$branch
	git push $repos HEAD:refs/for/$branch
	firefox http://review.tizen.org/gerrit/#q,$commitid,n,z
else
	echo "git push $repos HEAD:refs/for/$branch"
	git push --receive-pack='git receive-pack --reviewer tkq.kim@samsung.com' $repos HEAD:refs/for/$branch
	#git push $repos HEAD:refs/for/$branch
	firefox http://gerrit.score/#/q/$commitid
fi
git pull
...
set fd [open /home/jiil/bin/password r]
...
#!/bin/bash
cwd=`pwd`
show_help="build -c:clean, -d:debug, -r:release, -t:test, -i:include"
...
while getopts "h?cdrti" opt; do
    case "$opt" in
        ...
        d)
            ...
            cmake .. -DCMAKE_INSTALL_PREFIX=/home/jiil/git/install -DENABLE_TOOLS=On -DCMAKE_BUILD_TYPE=DEBUG
            ln -sf $PWD/compile_commands.json ..
            ;;
        r)
            echo "BUILD MODE RELEASE"
            cmake .. -DCMAKE_INSTALL_PREFIX=/home/jiil/git/install -DENABLE_TOOLS=On -DCMAKE_BUILD_TYPE=RELEASE
            ln -sf $PWD/compile_commands.json ..
            ;;
        t)
            ctest="ctest -V"
...
if [ "$include" = true ]
then
    echo "Make Find All Symbols for Clang Include Fixer"
    /usr/lib/llvm-10/share/clang/run-find-all-symbols.py -b 'find-all-symbols-10' -j4
