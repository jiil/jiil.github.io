...
module ORIS
  ...
  def self.getName(prefix, a=nil, b=nil, c=nil, d=nil, e=nil, f=nil, g=nil)
    name = prefix 
    name += ( a ? "_" + to_nm(a).to_s : "")
    name += ( b ? "_" + to_nm(b).to_s : "")
    name += ( c ? "_" + to_nm(c).to_s : "")
    name += ( d ? "_" + to_nm(d).to_s : "")
    name += ( e ? "_" + to_nm(e).to_s : "")
    name += ( f ? "_" + to_nm(f).to_s : "")
    name += ( g ? "_" + to_nm(g).to_s : "")
    return name
  end
  ...
  def self.genDiamondCell(size)
    cell_idx = getLayout.add_cell(getName("diamond", size))
    pts = []
  ...
  def self.genCircleCell(size, cnt)
    cell_idx = getLayout.add_cell(getName("circle", size, cnt))
    pts = []
    da = Math::PI * 2/cnt
    cnt.times do |i|
      pts.push(Point.from_dpoint(DPoint.new(size/2 * Math::cos(i* da) + size/2, size/2 * Math::sin(i*da) + size/2)))
    end
    getLayout.cell(cell_idx).shapes(getLayer).insert(Polygon.new(pts))
    return cell_idx
  end
